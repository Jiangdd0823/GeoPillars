...
class TransFusionHead(nn.Module):
    ...
    def predict(self, inputs):      # torch.Size([2, 512, 180, 180])
        batch_size = inputs.shape[0]
        lidar_feat = self.shared_conv(inputs)       # torch.Size([2, 128, 180, 180])

        lidar_feat_flatten = lidar_feat.view(       # torch.Size([2, 128, 32400])
            batch_size, lidar_feat.shape[1], -1
        )
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(lidar_feat.device)   # torch.Size([2, 32400, 2])
        ...
        # todo cross_attn
        query_feat = self.decoder(      # pos坐标 体素坐标+0.5
            query_feat, lidar_feat_flatten, query_pos, bev_pos
        )
        """
        query_feat:     torch.Size([2, 128, 200])
        lidar_feat_flatten:  torch.Size([2, 128, 32400])
        query_pos:  torch.Size([2, 200, 2])
        bev_pos:    torch.Size([2, 32400, 2])
        """
...
class TransFusionHead_sparsedet(nn.Module):
    """
        This module implements TransFusionHead 原来基础上修改.
        The code is adapted from https://github.com/mit-han-lab/bevfusion/ with minimal modifications.
    """
    ...
    def __init__(
        ...
        super(TransFusionHead_sparsedet, self).__init__()
    ...
    def predict(self, batch_dict):   # revise 整个修改
        # batch_size = inputs.shape[0]
        ...
        query_feat = batch_dict['q_feat'].permute(1, 2, 0)
        lidar_feat_flatten = batch_dict['kv_feat'].permute(1, 2, 0)
        query_pos = batch_dict['q_indices'].permute(1, 0, 2)[..., 1:]+0.5
        bev_pos = batch_dict['kv_indices'].permute(1, 0, 2)[..., 1:]+0.5
        query_heatmap_score = batch_dict['query_heatmap_score'].permute(2, 1, 0)
        dense_heatmap = batch_dict['heatmap_dense']
        self.query_labels = batch_dict['query_labels']

        # heatmap = dense_heatmap.detach().sigmoid() # sigmoid是为了得到热值01得分，传给res_layer不需要

        # todo cross_attn
        query_feat = self.decoder(  # pos坐标 体素坐标+0.5
            query_feat, lidar_feat_flatten, query_pos.float(), bev_pos.float()
        )
        res_layer = self.prediction_head(query_feat)
        res_layer["center"] = res_layer["center"] + query_pos.permute(0, 2, 1)

        res_layer["query_heatmap_score"] = query_heatmap_score      # fixme 可能不行
        res_layer["dense_heatmap"] = dense_heatmap
    ...
    def forward(self, batch_dict):
        # feats = batch_dict['spatial_features_2d']
        res = self.predict(batch_dict)  # revise
        if not self.training:
            bboxes = self.get_bboxes(res)
            batch_dict['final_box_dicts'] = bboxes
        else:
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, pred_dicts, **kwargs):
        ...
        one_hot_targets = torch.zeros(*list(labels.shape), self.num_classes + 1, dtype=cls_score.dtype,
                                      device=labels.device)
        one_hot_targets.scatter_(-1, labels.unsqueeze(dim=-1).long(), 1.0)
        one_hot_targets = one_hot_targets[..., :-1]
        loss_cls = self.loss_cls(
