...
class ProposalTargetLayer(nn.Module):
    ...
    def subsample_rois(self, max_overlaps):
        ...
        fg_inds = ((max_overlaps >= fg_thresh)).nonzero().view(-1)                                      # 阈值大于0.55的前景点
        easy_bg_inds = ((max_overlaps < self.roi_sampler_cfg.CLS_BG_THRESH_LO)).nonzero().view(-1)      # 阈值小于0.1的easy 背景点
        hard_bg_inds = ((max_overlaps < self.roi_sampler_cfg.REG_FG_THRESH) &
                (max_overlaps >= self.roi_sampler_cfg.CLS_BG_THRESH_LO)).nonzero().view(-1)             # 阈值大于0.1且大于0.55的hard 背景点
