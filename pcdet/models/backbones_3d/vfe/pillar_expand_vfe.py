...
class PillarExpandVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        self.with_multi_info = self.model_cfg.WITH_MULTI_INFO
        num_point_features += 6 if self.use_absolute_xyz else 3
        if self.with_distance:
            num_point_features += 1
        if self.with_multi_info:
    ...
    def point_dist_pos(self, point_coords):
        coords_square = point_coords.pow(2).sum(dim=2, keepdim=True)
        dist_squared = coords_square + coords_square.permute(0, 2, 1) - 2 * point_coords.float() @ point_coords.permute(0, 2, 1).float()
        dist_matrix = dist_squared.sqrt()
        f_dist_mean = torch.mean(dist_matrix, dim=-1, keepdim=True)
        return dist_matrix
