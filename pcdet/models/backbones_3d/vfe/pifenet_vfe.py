...
class PillarFeatureDANet(VFETemplate):
    def __init__(self, model_cfg, num_point_features, point_cloud_range, voxel_size, **kwargs):
        """
        Pillar Feature Net.
        ...
        num_input_features = model_cfg.num_input_features
        use_norm = model_cfg.use_norm
        num_filters = model_cfg.num_filters
        with_distance = model_cfg.with_distance
        voxel_size = voxel_size
        pc_range = point_cloud_range
        boost_channel_dim = model_cfg.boost_channel_dim
        reduction_rate = model_cfg.reduction_rate
        num_point_per_voxel = model_cfg.num_point_per_voxel
        channel_att = model_cfg.channel_att
        point_att = model_cfg.point_att
        task_aware_att = model_cfg.task_aware_att
        pool_types = model_cfg.pool_types
        ...
        self.num_point_features = num_point_features   # todo 承接外面
    ...
    def forward(self, batch_dict):
        features, num_voxels = batch_dict['voxels'], batch_dict['voxel_num_points']
        coors = batch_dict['voxel_coords']        # fixme
        # num_voxels = coors.shape[0]     # fixme
...
class PillarFeatureDANet_backup(nn.Module):
    def __init__(self,
    ...
    # def forward(self, features, num_voxels, coors):
    def forward(self, batch_dict):
        features, voxel_num_points = batch_dict['voxels'], batch_dict['voxel_num_points']
        coors = features.indices        # fixme
        num_voxels = coors.shape[0]     # fixme
