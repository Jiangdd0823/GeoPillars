...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        x_conv1 = self.conv1(x)     # nu [1440, 1440, 41]  16
        x_conv2 = self.conv2(x_conv1)       # nu [720, 720, 21]  32
        x_conv3 = self.conv3(x_conv2)       # nu [360, 360, 11]  64
        x_conv4 = self.conv4(x_conv3)       # nu [180, 180, 6]  128
        x_conv5 = self.conv5(x_conv4)       # nu [90, 90, 3]  128
        x_conv6 = self.conv6(x_conv5)       # nu [45, 45, 2]  128

        # x_conv5.indices[:, 1:] *= 2
        # x_conv6.indices[:, 1:] *= 4
        # revise 独立坐标，下面需返回原本的坐标
        stride1 = torch.cat([
            x_conv5.indices[:, 0].unsqueeze(1),
            x_conv5.indices[:, 1:] * 2
        ], dim=-1)
        stride2 = torch.cat([
            x_conv6.indices[:, 0].unsqueeze(1),
            x_conv6.indices[:, 1:] * 4
        ], dim=-1)
        # revise 下面x_conv4改成x_conv_out， 保证多尺度feat不变
        x_conv_out = x_conv4.replace_feature(torch.cat([x_conv4.features, x_conv5.features, x_conv6.features]))
        # x_conv_out.indices = torch.cat([x_conv4.indices, x_conv5.indices, x_conv6.indices])
        x_conv_out.indices = torch.cat([x_conv4.indices, stride1, stride2])

        out = self.bev_out(x_conv_out)         # sparse tensor [180, 180] 128
        ...
        batch_dict.update({
            'multi_scale_3d_features': {
                ...
                'x_conv3': x_conv3,
                'x_conv4': x_conv4,
                'x_conv5': x_conv5,         # revise 增加了x_conv5 x_conv6
                'x_conv6': x_conv6,
            }
        })
        batch_dict.update({
            'multi_scale_3d_strides': {
                ...
                'x_conv3': 4,
                'x_conv4': 8,
                'x_conv5': 16,  # revise 增加了x_conv5 x_conv6
                'x_conv6': 32,
            }
        })
